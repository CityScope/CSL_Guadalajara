...
global{
	...
	int address_who_send <- 1;
	int addres_to_send <- 2;
	...
	file map_file <- file("../includes/mexGuad.shp");//apple files
	file streets <- file("../includes/carr.shp"); //Streets files
...
species transport skills:[moving]{
	...
	init{
		ask block where(each.name = "block11"){
			myself.location <- location;
		}
		ask block where(each.name = "block0"){
